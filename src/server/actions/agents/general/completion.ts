...
export default async function GeneralAgentCompletion(
  ...
  try {
    ...
    if (!isValidJson(JSON.stringify(conversation))) {
      throw new Error(
        "Invalid JSON response from validateOrCreateConversation."
      );
    }
    ...
    if (
      !reply ||
      !totalTokens ||
      !isValidJson(JSON.stringify([reply, totalTokens]))
    ) {
      throw new Error(
    ...
    if (
      !isValidJson(JSON.stringify({ userId, turnId: turn.id, totalTokens }))
    ) {
    ...
    if (user?.autoGeneratedBucketId === null) {
      ...
      if (!newBucket || !isValidJson(JSON.stringify(newBucket))) {
        throw new Error(
          "Invalid JSON response from creating vocabulary bucket."
        );
      }
    ...
    if (useVocabAgent) {
      ...
      if (!vocabData.success)
        throw new Error(vocabData.message || "Vocabulary search error.");
      ...
      if (
        ...
        !isValidJson(JSON.stringify(vocabAgentSuggestion))
      ) {
        throw new Error(
      ...
      for (const word of enhanced_words) {
        ...
        if (!vocab || !isValidJson(JSON.stringify(vocab))) {
          throw new Error(
            `Invalid JSON response from creating vocabulary: ${word}`
          );
        }
      }
